...
Сперва реализуем отображение превью с помощью вёрстки и JS из первого задания. Для этого просто поместим собранные ранее билды `style.css` и `script.js` в папку `preview` и подключим скрипт `script.js` в `index.html`. Проверяем, всё работает корректно.

Теперь добавим линтер.
